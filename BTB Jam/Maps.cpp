	...
	if (inFile.is_open())
	{
		...
		for (int y = 0; y < nHeight; y++)
		{
			...
			for (int x = 0; x < nWidth; x++)
			{
				...
				{
				case '.':
					tiles[y * nWidth + x] = new cTile_Sky(x, y);
					break;
				case 'F':
					tiles[y * nWidth + x] = new cTile_Floor(x, y);
					break;
				case 'B':
					tiles[y * nWidth + x] = new cTile_Brick(x, y);
					break;
				case 'o':
					tiles[y * nWidth + x] = new cTile_Sky(x, y);
...
bool cMap_Level1::OnInteraction(vector<cDynamic*>& vecDyns, cDynamic* target, NATURE nature)
{
	...
	//If interact with object, start panning camera again
